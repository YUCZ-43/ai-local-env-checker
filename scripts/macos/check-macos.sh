...
LOG_FILE="$LOG_DIR/run-$TS-macos.log"
JSON_REPORT="$REPORT_DIR/report-$TS-macos.json"
MD_REPORT="$REPORT_DIR/report-$TS-macos.md"
...
detect_macos_networksetup() {
  local services service web secure socks output host port enabled value
  if ! command -v networksetup >/dev/null 2>&1; then
    return
  fi
  services="$(run_capture networksetup -listallnetworkservices | tail -n +2)"
  while IFS= read -r service; do
    [ -z "$service" ] && continue
    case "$service" in \**) service="${service#\*}" ;; esac
    for kind in web secureweb socksfirewall; do
      case "$kind" in
        web) output="$(run_capture networksetup -getwebproxy "$service")" ;;
        secureweb) output="$(run_capture networksetup -getsecurewebproxy "$service")" ;;
        socksfirewall) output="$(run_capture networksetup -getsocksfirewallproxy "$service")" ;;
      esac
      enabled="$(printf '%s\n' "$output" | awk -F': ' '/Enabled:/ {print $2; exit}')"
      host="$(printf '%s\n' "$output" | awk -F': ' '/Server:/ {print $2; exit}')"
      port="$(printf '%s\n' "$output" | awk -F': ' '/Port:/ {print $2; exit}')"
      if [ "$enabled" = "Yes" ] && [ -n "$host" ] && [ -n "$port" ] && [ "$port" != "0" ]; then
        case "$kind" in
          web) value="http://$host:$port" ;;
          secureweb) value="https://$host:$port" ;;
          socksfirewall) value="socks5://$host:$port" ;;
        esac
        printf '| networksetup:%s:%s | %s |\n' "$service" "$kind" "$(mask_proxy "$value")" >> "$MD_REPORT"
        [ -n "$MACOS_PROXY_JSON" ] && MACOS_PROXY_JSON="$MACOS_PROXY_JSON,"
        MACOS_PROXY_JSON="$MACOS_PROXY_JSON{\"service\":\"$(json_escape "$service")\",\"kind\":\"$kind\",\"value\":\"$(json_escape "$value")\"}"
        if [ -z "$RECOMMENDED_URL" ]; then
          RECOMMENDED_URL="$(mask_proxy "$value")"; RECOMMENDED_SOURCE="networksetup:$service:$kind"
          RECOMMENDED_PROTOCOL="$(printf '%s' "$value" | sed -E 's#^([A-Za-z0-9+.-]+)://.*#\1#')"
          RECOMMENDED_CONFIDENCE="medium"; RECOMMENDED_USABLE="true"; RECOMMENDED_PORT="$port"
        fi
      fi
    done
  done <<EOF
...
log "macOS check started language=$LANGUAGE timeout=$TIMEOUT_SEC skip_network=$SKIP_NETWORK"

cat > "$MD_REPORT" <<MD
# macOS Environment Check Report
...
if command -v sw_vers >/dev/null 2>&1; then
  mac_version="$(run_capture sw_vers -productVersion)"
else
  mac_version="unknown"
fi
arch="$(uname -m 2>/dev/null || true)"
printf '| macOS | OK | %s |\n' "$mac_version" >> "$MD_REPORT"
printf '| Architecture | OK | %s |\n' "$arch" >> "$MD_REPORT"
append_json_item "{\"name\":\"macos_version\",\"status\":\"OK\",\"value\":\"$(json_escape "$mac_version")\"}"
append_json_item "{\"name\":\"architecture\",\"status\":\"OK\",\"value\":\"$(json_escape "$arch")\"}"

if [ "$arch" = "arm64" ]; then
  if /usr/bin/pgrep oahd >/dev/null 2>&1; then rosetta="installed/running"; else rosetta="not detected"; fi
  printf '| Rosetta 2 | OK | %s |\n' "$rosetta" >> "$MD_REPORT"
fi

if command -v xcode-select >/dev/null 2>&1; then
  xcode_path="$(run_capture xcode-select -p)"
  printf '| Xcode CLT | OK | %s |\n' "$xcode_path" >> "$MD_REPORT"
fi

printf '\n## Tools\n\n| Tool | Status | Value |\n|------|--------|-------|\n' >> "$MD_REPORT"
for tool in brew node npm npx git code claude codex docker zsh bash; do check_tool "$tool" "$tool" "--version"; done
[ -x /usr/local/bin/brew ] && printf '| brew-intel-path | OK | /usr/local/bin/brew |\n' >> "$MD_REPORT"
[ -x /opt/homebrew/bin/brew ] && printf '| brew-arm-path | OK | /opt/homebrew/bin/brew |\n' >> "$MD_REPORT"
...
cat > "$JSON_REPORT" <<JSON
{
  ...
  "Proxy": {
    ...
    "Npm": {},
    "Git": {},
    "MacOSNetworkSetup": [$MACOS_PROXY_JSON],
