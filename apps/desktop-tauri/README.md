...
This may create local `.exe` or installer artifacts under `src-tauri/target/`. They are ignored and must not be committed, uploaded, tagged, or released in v0.7.0.
...
The CLI call layer is ready for `ai-local-deploy` and resolves the binary in this order: bundled packaged CLI path, `AI_LOCAL_DEPLOY_BIN`, local development CLI binary, then `go run .` under `apps/cli-go` for development only.
